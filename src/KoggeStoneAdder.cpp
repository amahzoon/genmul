...
int KoggeStoneAdder(int nIn1, int nIn2, string &file) // Get two integer numbers as input sizes and create the carry look ahead adder
{
    ...
    int N = P.size();                  //Size of the Adder
    int nNeededLevels = ceil(log2(N)); //the number of required levels for Lander-Fischer adders

    vector<PartialProduct> tempIn1; //for two vector concatination
    vector<PartialProduct> tempIn2; //for two vector concatination
    map<int, Component*> compLevelMap; //for keep track of component in grid
    /////////////////////////////////////////////
    for (int i = 0; i < nNeededLevels; i++) //Kogge-Stone algorithm
    {
        for (int j = pow(2, i); j < N; j++) // the grid substitution should be done at the end of each loop
        {
            tempIn1 = grid[j - pow(2, i)];
            tempIn2 = grid[j];
            ...
            comp->SetOutputs();
            compList.push_back(comp);
            compLevelMap[j] = comp; //for storing the component at each level
        }
        for (int j = pow(2, i); j < N; j++) //for grid substitution
            grid[j] = compLevelMap[j]->returnOutputs();
        compLevelMap.clear();
    }
    ///////////////////////////////////////////
    ...
    GenerateHeader(nIn1,nIn2, "KS", file);
