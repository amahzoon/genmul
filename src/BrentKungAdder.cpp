...
int BrentKungAdder(int nIn1, int nIn2, string &file) // Get two integer numbers as input sizes and create the carry look ahead adder
{
    ...
    /////////////////////////////////////////////
    for (int i = 0; i < nNeededLevels; i++) //Brent-Kung algorithm
    {
        for (int j = pow(2, i); j < N; j += pow(2, i + 1))
        {
            if (j + pow(2, i) - 1 < N)
            {
                tempIn1 = grid[j-1];
                tempIn2 = grid[j + pow(2, i) - 1];
                tempIn1.insert(tempIn1.end(), tempIn2.begin(), tempIn2.end()); //concatinating two vector

                comp = new CarryOperator(tempIn1);
                comp->SetOutputs();
                compList.push_back(comp);
                grid[j + pow(2, i) - 1] = comp->returnOutputs(); //updating grid values*/
            }
        }
    }

    for (int i = nNeededLevels - 2; i >= 0; i--)
    {
        for (int j = pow(2, i + 1); j < N; j += pow(2, i + 1))
        {
            if (j + pow(2, i) - 1 < N)
            {
                tempIn1 = grid[j-1];
    ...
    GenerateHeader(nIn1,nIn2, "BK", file);
