...
vector<int> Wallace5(map<int, int> Ins, int nIn1, int nIn2, string &file) // Get two integer numbers as input sizes and create the Wallace Tree PPA
{
    UNUSED(nIn1);
    UNUSED(nIn2);
    int inputNumber = 0;
    ...
    Component *comp1; //a temp component to store a just created component
    ...
    while (!stopFlag)
    {
        ...
        {
            iterationNum = (i.second.size())/7;
            for (int n=0; n<iterationNum; n++)
            {
                comp = new Counter ({i.second[0], i.second[1], i.second[2], i.second[3], i.second[4], i.second[5], i.second[6]});
                comp->SetOutputs();
                i.second.erase(i.second.begin(), i.second.begin()+7);
                compList.push_back(comp);
                GeneratedAtLevel.push_back(comp->returnOutputs()[0]);
                GeneratedAtLevel.push_back(comp->returnOutputs()[1]);
                GeneratedAtLevel.push_back(comp->returnOutputs()[2]);
            }
            if (i.second.size()==6)
            {
                comp = new FullAdder ({i.second[0], i.second[1], i.second[2]});
                comp1 = new FullAdder ({i.second[3], i.second[4], i.second[5]});
                comp->SetOutputs();
                comp1->SetOutputs();
                i.second.erase(i.second.begin(), i.second.begin()+6);
                compList.push_back(comp);
                compList.push_back(comp1);
                GeneratedAtLevel.push_back(comp->returnOutputs()[0]);
                GeneratedAtLevel.push_back(comp->returnOutputs()[1]);
                GeneratedAtLevel.push_back(comp1->returnOutputs()[0]);
                GeneratedAtLevel.push_back(comp1->returnOutputs()[1]);
            }
            if (i.second.size()==5)
            {
                comp = new FullAdder ({i.second[0], i.second[1], i.second[2]});
                comp1 = new HalfAdder ({i.second[3], i.second[4]});
                comp->SetOutputs();
                comp1->SetOutputs();
                i.second.erase(i.second.begin(), i.second.begin()+5);
                compList.push_back(comp);
                compList.push_back(comp1);
                GeneratedAtLevel.push_back(comp->returnOutputs()[0]);
                GeneratedAtLevel.push_back(comp->returnOutputs()[1]);
    ...
    GenerateHeader(Ins.size(), "CWT", file);
