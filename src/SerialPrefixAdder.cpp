...
int SerialPrefixAdder(int nIn1, int nIn2, string &file) // Get two integer numbers as input sizes and create the carry look ahead adder
{
    ...
    int nNeededLevels = P.size(); //the number of required levels for Serial Prefix adders

    vector<PartialProduct> tempIn1; //for two vector concatination
    vector<PartialProduct> tempIn2; //for two vector concatination

    /////////////////////////////////////////////
    for (auto i = 0; i < nNeededLevels-1; i++) //Serial Prefix algorithm
    {
        tempIn1 = grid[i];
        tempIn2 = grid[i + 1];
        ...
        comp->SetOutputs();
        compList.push_back(comp);
        grid[i+1] = comp->returnOutputs(); //updating grid values*/
    }
    ///////////////////////////////////////////
    ...
    GenerateHeader(nIn1, nIn2, "SE", file);
